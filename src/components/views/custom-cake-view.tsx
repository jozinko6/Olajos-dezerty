...
import { Cake, Calendar, Users, Euro, Truck, Store, Check } from 'lucide-react'
...
export function CustomCakeView() {
  ...
  const [form, setForm] = useState({
    ...
    eventType: 'Narodeniny', eventDate: '', guestCount: 10,
    budgetFrom: '', budgetTo: '', flavors: '', message: '',
    deliveryType: 'PICKUP', deliveryCity: '', deliveryAddress: '',
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (!form.message || form.message.length < 5) {
      toast.error('Opíšte vašu predstavu.')
      return
    }
    setSubmitting(true)
    try {
      await api.post('/api/custom-cake', {
        ...form,
      ...
      setDone(true)
      toast.success('Dopyt bol odoslaný. Ozveme sa vám do 24 hodín.')
    } catch (e: any) {
      ...
      <div className="container mx-auto px-4 py-20">
        <Card className="max-w-lg mx-auto p-10 text-center bg-[var(--card)] border-[var(--gold)]/30">
          ...
          <p className="text-[var(--muted-foreground)] mb-6">
            Váš dopyt na tortu na mieru sme prijali. Naša cukrárka sa vám ozve do 24 hodín s cenovým návrhom.
          </p>
      ...
      <div className="max-w-2xl mx-auto">
        <div className="text-center mb-8">
          <div className="olajos-divider mb-4">
            <span className="font-display text-sm uppercase tracking-widest">Torta na mieru</span>
          </div>
          <h1 className="font-display text-3xl md:text-4xl font-bold text-[var(--chocolate)] mb-3">Navrhneme tortu vašich snov</h1>
          <p className="text-[var(--muted-foreground)]">
            Vyplňte dopyt a my vám pripravíme cenový návrh na mieru. Minimálny predstih je 48 hodín.
          </p>
        </div>
        ...
        <Card className="p-6 bg-[var(--card)] border-[var(--border)]">
          <form onSubmit={submit} className="space-y-5">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label className="text-[var(--chocolate)]">Typ udalosti *</Label>
                <select
                ...
                >
                  ...
                  <option>Promócie</option>
                  <option>Firemná akcia</option>
                  <option>Iné</option>
                </select>
              </div>
              <div className="space-y-2">
                <Label className="text-[var(--chocolate)] flex items-center gap-1"><Calendar className="h-3 w-3" /> Dátum udalosti *</Label>
            ...
            <div className="grid sm:grid-cols-3 gap-4">
              <div className="space-y-2">
                <Label className="text-[var(--chocolate)] flex items-center gap-1"><Users className="h-3 w-3" /> Počet osôb *</Label>
              ...
              <div className="space-y-2">
                ...
                <Input type="number" min={0} value={form.budgetFrom} onChange={e => set('budgetFrom', e.target.value)} placeholder="30" />
              </div>
              <div className="space-y-2">
                <Label className="text-[var(--chocolate)] flex items-center gap-1"><Euro className="h-3 w-3" /> Rozpočet do (€)</Label>
                <Input type="number" min={0} value={form.budgetTo} onChange={e => set('budgetTo', e.target.value)} placeholder="100" />
              </div>
            </div>

            <div className="space-y-2">
              <Label className="text-[var(--chocolate)]">Chuť a dekorácia</Label>
              <Input value={form.flavors} onChange={e => set('flavors', e.target.value)} placeholder="čokoládová, smotanová, ovocie..." />
            </div>

            <div className="space-y-2">
              <Label className="text-[var(--chocolate)]">Opíšte vašu predstavu *</Label>
              <Textarea value={form.message} onChange={e => set('message', e.target.value)} rows={5} required placeholder="Farby, motív, nápis na torte, špeciálne požiadavky..." />
            </div>

            <div className="space-y-2">
              <Label className="text-[var(--chocolate)]">Spôsob prevzatia</Label>
              <RadioGroup value={form.deliveryType} onValueChange={(v) => set('deliveryType', v)} className="grid sm:grid-cols-2 gap-2">
                <label className={`flex items-center gap-3 border rounded-lg p-3 cursor-pointer ${form.deliveryType === 'PICKUP' ? 'border-[var(--gold)] bg-[var(--gold)]/5' : 'border-[var(--border)]'}`}>
                  <RadioGroupItem value="PICKUP" />
                  <Store className="h-4 w-4 text-[var(--gold)]" />
                  <span className="text-sm text-[var(--chocolate)]">Osobný odber</span>
                </label>
                <label className={`flex items-center gap-3 border rounded-lg p-3 cursor-pointer ${form.deliveryType === 'DELIVERY' ? 'border-[var(--gold)] bg-[var(--gold)]/5' : 'border-[var(--border)]'}`}>
                  <RadioGroupItem value="DELIVERY" />
                  <Truck className="h-4 w-4 text-[var(--gold)]" />
                  <span className="text-sm text-[var(--chocolate)]">Doručenie</span>
                </label>
              </RadioGroup>
            </div>
            ...
            {form.deliveryType === 'DELIVERY' && (
              <div className="grid sm:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  <Label className="text-[var(--chocolate)]">Adresa</Label>
                  <Input value={form.deliveryAddress} onChange={e => set('deliveryAddress', e.target.value)} placeholder="Ulica a číslo" />
                </div>
              </div>
            )}
            ...
            <Button type="submit" className="w-full bg-[var(--chocolate)] text-[var(--cream)] hover:bg-[var(--gold)] hover:text-[var(--chocolate)]" disabled={submitting}>
              <Cake className="h-4 w-4 mr-2" /> {submitting ? 'Odosielam...' : 'Odoslať dopyt'}
            </Button>
          </form>
        </Card>
      </div>
